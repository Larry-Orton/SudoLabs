...
from howl.ui.progress import render_attack_chain, render_hunt_status
from howl.ui.dashboard import render_score_dashboard, render_profile
from howl.targets.registry import TargetRegistry
from howl.db import queries
from howl.db.database import init_db
from howl.config import get_api_key, set_api_key, get_username, set_username
from howl.constants import get_rank

app = typer.Typer(
    name="howl",
    help="Howl - Terminal Cybersecurity Hacking Lab",
...
def main(ctx: typer.Context):
    """Launch the Howl interactive menu."""
    ...
    while True:
        ...
        if choice == "1":
            ...
            console.print("\n  [dim]The wolf rests... for now.[/dim]\n")
            raise typer.Exit()
...
def _launch_htb(ip: str, name: str | None, hostname: str | None, no_hosts: bool):
    ...
    from howl.htb.session import HtbSession
    from howl.htb.loop import htb_hunt_loop
    ...
    if hostname and not no_hosts:
        from howl.htb.hosts import add_host_entry
    ...
    if hostname and not no_hosts:
        if Confirm.ask(f"\n  Remove {hostname} from /etc/hosts?"):
            from howl.htb.hosts import remove_host_entry
...
def _launch_hunt(target_slug: str):
    ...
    from howl.engine.tracker import inject_flags_into_containers, run_post_start_commands, display_stage_info
    from howl.scoring.achievements import check_achievements
    ...
    try:
        ...
        console.print("  [dim]Run 'howl doctor' to diagnose issues.[/dim]\n")
...
def _hunt_loop(session, container_info: dict | None):
    ...
    from howl.engine.tracker import display_stage_info
    from howl.scoring.achievements import check_achievements
    ...
    bar = FixedBar(HUNT_COMMANDS, "howl")
    ...
    try:
        while not session.completed:
            ...
            if cmd == "clear":
                ...
                from howl.htb.loop import _build_history_context
                ...
                session.pause()
                console.print("  [yellow]Session paused. Resume with 'howl hunt {}'.".format(target.slug) + "[/yellow]")
                break
...
def hint(
    ...
    if not active:
        error_panel("No Active Hunt", "Start a hunt first with: howl hunt <target>")
        return
...
def _show_score(target_slug: str | None = None):
    ...
    if target_slug:
        ...
        from howl.ui.tables import render_score_table
        registry = TargetRegistry()
        target = registry.get_by_slug(target_slug)
        if not target:
            error_panel("Not Found", f"Target '{target_slug}' not found.")
...
@app.command()
def doctor():
    """Check system readiness for Howl."""
    _ensure_db()
    _run_doctor()
...
def _run_doctor():
    ...
    # Database
    from howl.config import DB_FILE
...
def config(
    ...
    """View or edit Howl configuration."""
    _ensure_db()
...
def _show_config(new_api_key: str | None = None):
    ...
    from howl.config import HOWL_HOME, DB_FILE, TARGETS_DIR

    console.print(f"\n  [bold]Howl Configuration[/bold]\n")
    console.print(f"  Howl Home:   {HOWL_HOME}")
...
def _interactive_ai_chat():
    ...
    if not ai.is_available():
        warning_panel("AI Unavailable", "Set your API key with: howl config --set-api-key")
        return
