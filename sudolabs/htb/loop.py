...
from sudolabs.ui.panels import info_panel, success_panel, warning_panel, hint_panel
from sudolabs.ui.terminal import FixedBar
from sudolabs.ai.helper import AIHelper
from sudolabs.htb.session import HtbSession, HtbMilestone, MILESTONE_LABELS, MILESTONE_ORDER
...
def _get_walkthrough_info(session: HtbSession, extra_query: str = "") -> str | None:
    ...
    from sudolabs.ai.websearch import search_walkthroughs
...
def htb_hunt_loop(session: HtbSession):
    ...
    bar = FixedBar(HTB_COMMANDS, "sudolabs/htb")
...
def _handle_scan(session: HtbSession, scan_type: str):
    ...
    from sudolabs.htb.scanner import is_nmap_available, run_nmap_scan
...
def _handle_hint(session: HtbSession, ai: AIHelper, level: int, command_history: list[dict] | None = None):
    ...
    from sudolabs.ai.htb_prompts import HTB_SYSTEM_PROMPT, build_htb_hint_prompt
...
def _handle_ask(session: HtbSession, ai: AIHelper, question: str, command_history: list[dict] | None = None):
    ...
    from sudolabs.ai.htb_prompts import HTB_SYSTEM_PROMPT, build_htb_chat_prompt
