"""Attack chain progress display for SudoLabs."""
...
def render_attack_chain(stages: list, current_stage: int, stage_scores: dict | None = None):
    ...
    for i, stage in enumerate(stages):
        ...
        if i < current_stage:
            ...
            style = "sudo.stage.active"
        else:
            # Locked
            bar = "[dim]-[/dim]" * 10
            score_text = "[dim]Locked[/dim]"
            icon = "[dim].[/dim]"
            style = "sudo.stage.locked"
...
def render_hunt_status(
    ...
    from sudolabs.constants import DIFFICULTY_COLORS, Difficulty
